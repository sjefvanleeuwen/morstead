...
namespace Vs.VoorzieningenEnRegelingen.Core.Tests
{
    public class ModelTests
    {
        ...
        private void ResolveToQuestion(ref YamlScriptController controller, Model.Model model, string name, ref List<ContentNode> items, string situation = null)
        {
            ...
            foreach (var function in formula.Functions)
            {
                ...
                foreach (var parameter in parameters)
                {
                    if (model.Formulas.Find(p => p.Name == parameter.Name) == null)
                    {
                        ...
                        items.Add(new ContentNode(string.Join('.', new[] { parameter.Name, function.Situation, name }.Where(s => !string.IsNullOrEmpty(s)))) { IsSituational = function.IsSituational, Situation = function.Situation, Parameter = parameter });
                        // find formula's that use the answer to this question.
                        ResolveToQuestion(ref controller, model, parameter.Name, ref items);
                    }
                    else
                    {
        ...
        public void BuildTreeForContent()
        {
            ...
            {
                ResolveToQuestion(ref controller, result.Model, step.Formula, ref items, step.Situation);
                // TODO: Resolve recht/geen recht.
                if (!string.IsNullOrEmpty(step.Break))
                {
                    ContentNode node = new ContentNode(string.Join('.',new[] { "geen_recht",step.Situation,step.Formula }.Where(s => !string.IsNullOrEmpty(s)))) { IsBreak = true,IsSituational=step.IsSituational,Situation=step.Situation, Parameter = new Parameter(name:"recht",value:null,type:TypeEnum.Boolean,model:ref model) };
                    items.Add(node);
                }
            }
