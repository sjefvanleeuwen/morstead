...
namespace Vs.Morstead.Tests.Content
{
    public class PublicationTests : IClassFixture<ClusterFixture>
    {
        ...
        public async void ShouldPersistPublication()
        {
            ...
            var grain = cluster.GrainFactory.GetGrain<IContentPersistentGrain>("did:vsoc:mstd:pub:Sp8WbN5r0E6MCEx54Is3oQ-3");
            var d = await grain.Load();
          // await grain.Save("application/yaml", EncodingType.UTF8, target);
          ...
          //  Assert.Equal(document.ContentAs<string>(), target);
        }
    }
}
